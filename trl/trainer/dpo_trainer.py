...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        #         train_dataset = HFDataset.from_list(train_dataset.list_data_dict)
        #     train_dataset = train_dataset.map(self.tokenize_row, num_proc=self.dataset_num_proc)
        #     if eval_dataset is not None:
        #         if not hasattr(eval_dataset, "map"):
        #             eval_dataset = HFDataset.from_list(eval_dataset.list_data_dict)
    ...
    def concatenated_inputs(
        ...
        concatenated_batch["concatenated_images"] = batch["images"] + batch["images_rejected"]
    ...
    # trl 0.12's compute_loss why? transformers 4.51.1 need this type of compute_loss
    def compute_loss(
        self,
        ...
        compute_loss_context_manager = torch.cuda.amp.autocast("cuda") if self._peft_has_been_casted_to_bf16 else nullcontext()
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if version.parse(transformers_version) >= version.parse("4.47.0.dev0"):
