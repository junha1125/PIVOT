...
FIN_RUN_NAME="pivot-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-llava"
echo "FIN_RUN_NAME: ${FIN_RUN_NAME}"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}"\
    ...
    --data_path ./datasets/data-processor/OneVisionData/single_image_cambrian.yaml \
    ...
    --pretrain_mm_mlp_adapter="./checkpoints/pivot-proj-train/${BASE_RUN_NAME}/mm_projector.bin" \
    --group_by_modality_length True \
    --output_dir "./checkpoints/pivot-full-train/${FIN_RUN_NAME}" \
    --num_train_epochs 1 \
    --save_strategy "steps" \
    --save_steps 500 \
    --save_total_limit 1 \
