...
def eval_model(args):
    ...
    if hf_path is None:
        raise RuntimeError("HF_PATH environment variable is not set. Please set HF_PATH to the cache directory for HuggingFace datasets.")
    for i in range(300):
        file_path = hf_hub_download(repo_id="MMVP/MMVP", filename=f"{i+1}.jpg", subfolder="MMVP Images", repo_type="dataset", cache_dir=hf_path)
        images[i] = Image.open(file_path).convert('RGB')

    questions = []
    file_path = hf_hub_download(repo_id="MMVP/MMVP", filename="Questions.csv", repo_type="dataset", cache_dir=hf_path)
