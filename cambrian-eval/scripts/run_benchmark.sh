...
# extract basename (last 2 or 3 path components, depending on 'checkpoint' keyword)
parts=(${ckpt//\// })
n=${#parts[@]}
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    echo python $eval_file --model_path "$ckpt" --num_chunks "$CHUNKS" --chunk_idx "$IDX" --answers_file "$answers_file" --conv_mode "$conv_mode"
    {
