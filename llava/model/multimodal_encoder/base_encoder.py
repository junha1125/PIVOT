...
class ProcessorWrapper:
    ...
    def preprocess(self, image, return_tensors='pt'):
        if isinstance(image, (list, tuple)):
            pixel_values = torch.stack([self._transforms(im) for im in image], dim=0)  # (B, C, H, W)
        else:
            pixel_values = self._transforms(image).unsqueeze(0)  # (1, C, H, W)
...
class BaseVisionTower(nn.Module):
    def __init__(self, vision_tower_name, args, delay_load=False):
        ...
        rank0_print(f"Unfreezing MM Vision Tower: {self.unfreeze_mm_vision_tower}")
