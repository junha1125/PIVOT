...
class MoCoVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        if os.path.exists(local_path):
            rank0_print(f"Loading `{self.vision_tower_name}` from local path: {local_path}")
            ckpt = torch.load(local_path, map_location=torch.device('cpu'))
        else:
            rank0_print(f"Downloading `{self.vision_tower_name}` from url: {url}")
        ...
        rank0_print(f"Loaded MoCo model: {self.vision_tower_name} with hidden size: {self._hidden_size}, image size: {self._image_size}, patch size: {self._patch_size}")
        ...
        self.target_dtype = None  # moco always uses float32
    ...
    def _forward(self, images):
        if self.target_dtype is None:
            self.target_dtype = next(self.vision_tower.parameters()).dtype
        # target_dtype = torch.float16
        with torch.set_grad_enabled(self.unfreeze_mm_vision_tower):
            images_fp32 = images.to(device=self.device, dtype=self.target_dtype, non_blocking=True)
            image_feature_outs = self.vision_tower.forward_features(images_fp32)

            image_features = self.feature_select(image_feature_outs)
