    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            if metrics is not None: 
                rank0_print(f"Trainer._save_checkpoint allows only two inputs, model and trial. metrics is deprecated in transformers 4.51.0")
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if len(lr_mapper) > 0:
                ...
                for module_keyword, lr in lr_mapper.items():
                    rank0_print(f"[LOG] Parameters of *{module_keyword}* are set as training with lr=*{lr}*")
                    module_parameters = [name for name, _ in opt_model.named_parameters() if module_keyword in name]
                    optimizer_grouped_parameters.extend(
                        [
                            {
